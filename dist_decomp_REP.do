...
*Option to drop newly added MSA
g flag = 0
replace flag = 1 if name_msa == ""
drop if flag == 1
...
egen g1 = mean(unaval), by(state)
egen g2 = mean(populat_saiz),  by(state)
egen g3 = mean(WRLURI),   by(state)
...
egen mean_WRI1 = pctile(WR) if flag == 0, p(50)
egen mean_WR = mean(mean_WRI1)
g        new_inver = inver
...
keep ddd* hhh* www* qqq*  emp2019 name msa brain rust south other alpha
